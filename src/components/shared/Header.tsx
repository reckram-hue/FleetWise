  ...
  return (
    <header className="bg-gray-800 text-white shadow-lg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between min-h-16 gap-2 py-3">
          <div className="flex items-center min-w-0">
            {onBack && (
              <button
                onClick={onBack}
                className="flex items-center px-3 py-2 rounded-lg text-gray-300 hover:text-white hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white mr-2 transition-colors"
            ...
            <ShieldCheck className="hidden sm:block shrink-0 h-8 w-8 text-blue-400" />
            <h1 className="text-lg sm:text-xl font-bold sm:ml-3">{title}</h1>
          </div>
          <div className="flex items-center">
            <span className="hidden md:inline text-gray-300 mr-4">Welcome, {currentUser?.firstName} {currentUser?.surname}</span>
            <button
