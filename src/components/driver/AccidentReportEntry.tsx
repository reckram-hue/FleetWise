...
export default function AccidentReportEntry({ assignmentId }: { assignmentId: string }) {
    ...
    useEffect(() => {
        ...
        accidentApi.list(assignmentId).then(r => { if (!cancelled) setReports(r); })
    ...
    return <section className="my-4 rounded-xl border-2 border-red-800 p-4 bg-red-50">
        ...
        <button className="min-h-11 my-2 rounded bg-red-800 text-white px-4 py-2 font-bold disabled:opacity-50" disabled={loading || !!error} onClick={() => open()}>Report Accident / Collision</button>
        ...
        {selected && <div className="fixed inset-0 z-50 bg-white overflow-y-auto"><AccidentReportForm key={selected.id} initialReport={selected} onBack={() => { setSelected(null); setVersion(v => v + 1); }} /></div>}
    </section>;
}
