...
import { Car, Loader, Search } from 'lucide-react';

export type VehiclePick = { id: string; registration: string; alias?: string; vehicleType: 'ICE' | 'EV' };
...
const toPick = (v: Vehicle): VehiclePick => ({ id: v.id, registration: v.registration, alias: v.alias, vehicleType: v.vehicleType });
...
const TakeVehicleForm: React.FC<TakeVehicleFormProps> = ({
  ...
  const [startOdo, setStartOdo] = useState(suggestedStartOdo != null ? String(suggestedStartOdo) : '');
  ...
  const loadAvailable = async () => {
    ...
    try {
      ...
      setVehicles(all.filter(v => v.status === 'Active' && !(v as any).activeAssignmentId));
    } catch {
    ...
    <div className='space-y-4'>
      ...
      {selectedVehicle ? (
        <Card>
          ...
          <div className='space-y-4'>
            <div>
              <label className='block text-sm font-semibold text-gray-700 mb-1'>Starting Odometer (km) *</label>
        ...
        <Card>
          ...
          {suggestedVehicle && (
            <button onClick={() => { setSelectedVehicle(suggestedVehicle); setStartOdo(suggestedStartOdo != null ? String(suggestedStartOdo) : ''); setStartCharge(suggestedStartChargePercent != null ? String(suggestedStartChargePercent) : ''); }} className='w-full mb-3 p-3 bg-blue-50 border border-blue-300 rounded-lg text-left hover:bg-blue-100'>
          ...
          <div className='space-y-2 max-h-72 overflow-y-auto'>
            {filtered.length === 0 ? (
              ...
              <button key={v.id} onClick={() => { setSelectedVehicle(toPick(v)); setStartOdo(''); setStartCharge(''); }} className='w-full text-left p-3 border rounded-lg hover:border-blue-500 hover:bg-blue-50 flex justify-between items-center'>
                <span>
