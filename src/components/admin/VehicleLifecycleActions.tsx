...
export default function VehicleLifecycleActions({ vehicle, onSaved }: { vehicle: Vehicle; onSaved: () => void }) {
  ...
  async function save() {
    ...
    try { await api.changeVehicleLifecycleAdmin({ vehicleId: vehicle.id, status, notes, clearManualHold, requestId });
      setRequestId(crypto.randomUUID()); onSaved();
  ...
  return <div className="border rounded p-3 my-3">
    <h4 className="font-semibold">Operational lifecycle: {vehicle.status}</h4>
