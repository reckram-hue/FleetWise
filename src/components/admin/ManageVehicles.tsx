...
import { Plus, Edit, Trash2, X, Wrench, QrCode, ChevronDown, RefreshCw, Search } from 'lucide-react';
...
const ManageVehicles: React.FC<ManageVehiclesProps> = ({ onBack }) => {
    ...
    const [customFinanceInput, setCustomFinanceInput] = useState(false);
    const [customInsuranceInput, setCustomInsuranceInput] = useState(false);
    const [customTrackingInput, setCustomTrackingInput] = useState(false);
    const [customWarrantyInput, setCustomWarrantyInput] = useState(false);
    ...
    const fetchServiceProviders = async () => {
        try {
            const providers = await api.getServiceProviders(true); // Only get active providers
    ...
    const syncVehiclesWithBackend = async () => {
        try {
            setLoading(true);
            const allVehicles = await api.getVehicles();
            let synced = 0;
            ...
            for (const vehicle of allVehicles) {
                try {
                    await fetch('http://localhost:3001/api/vehicles', {
                        ...
                        body: JSON.stringify({
                            id: vehicle.id,
                            name: vehicle.alias || vehicle.registration || vehicle.id
                        })
                    });
                    synced++;
                } catch (err) {
                    console.error(`Failed to sync vehicle ${vehicle.id}:`, err);
                }
            }

            alert(`✅ Successfully synced ${synced} of ${allVehicles.length} vehicles with backend server.`);
        } catch (error) {
            console.error('Failed to sync vehicles:', error);
            alert('❌ Failed to sync vehicles. Make sure the backend server is running on port 3001.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAddClick = () => {
        ...
        setCustomFinanceInput(false);
        setCustomInsuranceInput(false);
        setCustomTrackingInput(false);
        setCustomWarrantyInput(false);
        setShowForm(true);
    };

    const handleEditClick = (vehicle: Vehicle) => {
        setIsEditing(true);
        setIsEditMode(true); // Start in edit mode for existing vehicles
        setSelectedVehicle(vehicle);
        // Check if current values are custom (not in the provider lists)
        setCustomFinanceInput(isCustomProvider(vehicle.financeCompany, 'Finance'));
        setCustomInsuranceInput(isCustomProvider(vehicle.insuranceCompany, 'Insurance'));
        setCustomTrackingInput(isCustomProvider(vehicle.trackingCompany, 'Tracking'));
        setCustomWarrantyInput(isCustomProvider(vehicle.warrantyInsurer, 'Warranty'));
        setShowForm(true);
    };
    ...
    const sortedVehicles = [...vehicles]
        .filter(v => v != null) // Filter out null/undefined entries
        .filter(v => {
        ...
        .sort((a, b) => {
            ...
            if (statusSortOrder === 'default') {
                // Default sort by registration with null safety
    ...
    const handleFormChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        if (name === 'financeCompany') {
            if (value === '__CUSTOM__') {
                setCustomFinanceInput(true);
                setSelectedVehicle({ ...selectedVehicle, financeCompany: '' });
                return;
            } else {
                setCustomFinanceInput(false);
            }
        ...
        if (name === 'insuranceCompany') {
            if (value === '__CUSTOM__') {
                setCustomInsuranceInput(true);
                setSelectedVehicle({ ...selectedVehicle, insuranceCompany: '' });
                return;
            } else {
                setCustomInsuranceInput(false);
            }
        }
        if (name === 'trackingCompany') {
            if (value === '__CUSTOM__') {
                setCustomTrackingInput(true);
                setSelectedVehicle({ ...selectedVehicle, trackingCompany: '' });
                return;
            } else {
                setCustomTrackingInput(false);
            }
        }
        if (name === 'warrantyInsurer') {
            if (value === '__CUSTOM__') {
                setCustomWarrantyInput(true);
                setSelectedVehicle({ ...selectedVehicle, warrantyInsurer: '' });
                return;
            } else {
                setCustomWarrantyInput(false);
            }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Validate that custom provider fields are not left empty when in custom mode
        if (customFinanceInput && !selectedVehicle.financeCompany?.trim()) {
            alert('Please enter a custom finance company name or click Cancel to select from the list.');
            return;
        }
        if (customInsuranceInput && !selectedVehicle.insuranceCompany?.trim()) {
            alert('Please enter a custom insurance company name or click Cancel to select from the list.');
            return;
        }
        if (customTrackingInput && !selectedVehicle.trackingCompany?.trim()) {
            alert('Please enter a custom tracking company name or click Cancel to select from the list.');
            return;
        }
        if (customWarrantyInput && !selectedVehicle.warrantyInsurer?.trim()) {
            alert('Please enter a custom warranty insurer name or click Cancel to select from the list.');
            return;
        }

        try {
            // Ensure status is always set (default to Active if missing)
            let vehicleToSave: any = {
                ...selectedVehicle,
                status: selectedVehicle.status || VehicleStatus.Active,
                statusDate: selectedVehicle.statusDate || new Date().toISOString().split('T')[0]
            };

            // Clean up vehicle-type-specific fields (Firestore doesn't support undefined values)
            if (vehicleToSave.vehicleType === VehicleType.EV) {
                // For EVs, remove ICE-specific fields completely
                delete vehicleToSave.fuelType;
                delete vehicleToSave.manufacturerFuelConsumption;
                delete vehicleToSave.baselineFuelConsumption;
                delete vehicleToSave.currentFuelConsumption;
            } else {
            ...
            // Remove any remaining undefined values to prevent Firestore errors
            Object.keys(vehicleToSave).forEach(key => {
                if (vehicleToSave[key] === undefined) {
                    delete vehicleToSave[key];
                }
            });

            if (isEditing) {
                await api.updateVehicle(vehicleToSave as Vehicle);
            } else {
                await api.addVehicle(vehicleToSave as Omit<Vehicle, 'id'>);
            }
            closeForm();
            fetchVehicles();
        } catch (error) {
            console.error("Failed to save vehicle:", error);
            alert("Error saving vehicle details: " + (error as Error).message);
        }
    };

    const getProvidersBySpecialization = (specialization: string): ServiceProvider[] => {
        return serviceProviders.filter(provider =>
            provider.specializations.includes(specialization) && provider.isActive
        );
    };

    const isCustomProvider = (currentValue: string | undefined, specialization: string): boolean => {
        if (!currentValue || currentValue === '') return false;
        const providers = getProvidersBySpecialization(specialization);
        return !providers.some(p => p.name === currentValue);
    };
    ...
    const getNextServiceKm = (vehicle: Vehicle) => {
        // Check if service interval is configured
        if (!vehicle.serviceIntervalKm || vehicle.serviceIntervalKm <= 0) {
            return 'N/A';
        }
                    ...
                    {!isEditing ? (
                        ...
                        <div className="flex items-center justify-between w-full">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={closeForm} className="p-1 rounded-full hover:bg-gray-200"><X size={20} /></button>
                            </div>
                        </div>
                    )}
                    {!isEditing && (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                {!customFinanceInput ? (
                                    <select
                                        name="financeCompany"
                                        value={selectedVehicle.financeCompany || ''}
                                        onChange={handleFormChange}
                                        className="mt-1 p-2 border rounded w-full"
                                        disabled={!isEditMode}
                                    >
                                        <option value="">Select finance provider...</option>
                                        {getProvidersBySpecialization('Finance').map(provider => (
                                            <option key={provider.id} value={provider.name}>{provider.name}</option>
                                        ))}
                                        <option value="__CUSTOM__">+ Add Custom Provider</option>
                                    </select>
                                ) : (
                                    <div className="flex gap-2">
                                        <input
                                            type="text"
                                            value={selectedVehicle.financeCompany || ''}
                                            placeholder="Enter custom finance company name"
                                            onChange={(e) => setSelectedVehicle({ ...selectedVehicle, financeCompany: e.target.value })}
                                            className="mt-1 p-2 border rounded w-full"
                                            disabled={!isEditMode}
                                            autoFocus
                                        />
                                        <button
                                            ...
                                            className="mt-1 px-3 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300"
                                            disabled={!isEditMode}
                                        >
                                            Cancel
                                        </button>
                            ...
                            <div>
                                ...
                                {!customInsuranceInput ? (
                                    <select
                                        ...
                                        value={selectedVehicle.insuranceCompany || ''}
                                        onChange={handleFormChange}
                                        className="mt-1 p-2 border rounded w-full"
                                        disabled={!isEditMode}
                                    >
                                        <option value="">Select insurance provider...</option>
                                        {getProvidersBySpecialization('Insurance').map(provider => (
                                            <option key={provider.id} value={provider.name}>{provider.name}</option>
                                        ))}
                                        <option value="__CUSTOM__">+ Add Custom Provider</option>
                                    </select>
                                    ...
                                    <div className="flex gap-2">
                                        <input
                                            ...
                                            onChange={(e) => setSelectedVehicle({ ...selectedVehicle, insuranceCompany: e.target.value })}
                                            className="mt-1 p-2 border rounded w-full"
                                            disabled={!isEditMode}
                                            autoFocus
                                        />
                            ...
                            <div>
                                ...
                                {!customTrackingInput ? (
                                    <select
                                        ...
                                        value={selectedVehicle.trackingCompany || ''}
                                        onChange={handleFormChange}
                                        className="mt-1 p-2 border rounded w-full"
                                        disabled={!isEditMode}
                                    >
                                        <option value="">Select tracking provider...</option>
                                        {getProvidersBySpecialization('Tracking').map(provider => (
                                            <option key={provider.id} value={provider.name}>{provider.name}</option>
                                        ))}
                                        <option value="__CUSTOM__">+ Add Custom Provider</option>
                                    </select>
                                    ...
                                    <div className="flex gap-2">
                                        <input
                                            type="text"
                                            value={selectedVehicle.trackingCompany || ''}
                                            placeholder="Enter custom tracking company name"
                                            onChange={(e) => setSelectedVehicle({ ...selectedVehicle, trackingCompany: e.target.value })}
                                            className="mt-1 p-2 border rounded w-full"
                                            disabled={!isEditMode}
                                            autoFocus
                                        />
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                {!customWarrantyInput ? (
                                    <select
                                        name="warrantyInsurer"
                                        value={selectedVehicle.warrantyInsurer || ''}
                                        onChange={handleFormChange}
                                        className="mt-1 p-2 border rounded w-full"
                                        disabled={!isEditMode}
                                    >
                                        <option value="">Select warranty provider...</option>
                                        {getProvidersBySpecialization('Warranty').map(provider => (
                                            <option key={provider.id} value={provider.name}>{provider.name}</option>
                                        ))}
                                        <option value="__CUSTOM__">+ Add Custom Provider</option>
                                    </select>
                                    ...
                                    <div className="flex gap-2">
                                        <input
                                            type="text"
                                            value={selectedVehicle.warrantyInsurer || ''}
                                            placeholder="Enter custom warranty insurer name"
                                            onChange={(e) => setSelectedVehicle({ ...selectedVehicle, warrantyInsurer: e.target.value })}
                                            className="mt-1 p-2 border rounded w-full"
                                            disabled={!isEditMode}
                                            autoFocus
                                        />
                ...
                <>
                    <div className="mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
                        <div className="flex flex-col md:flex-row gap-2 w-full md:w-auto items-center">
                            ...
                            <button onClick={handleAddClick} className="bg-blue-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-blue-600 transition flex items-center gap-2 whitespace-nowrap">
                                <Plus size={16} />
                                Add Vehicle
                            </button>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button
                                ...
                                className={`px-4 py-2 border rounded-lg transition-colors flex items-center gap-2 ${statusSortOrder !== 'default'
                                    ...
                                    return (
                                        <React.Fragment key={vehicle.id}>
                                            ...
                                            <tr className={`hover:bg-gray-50 ${isFirstOfStatus && statusSortOrder !== 'default' ? 'border-t-2 border-blue-200' : ''}`}>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${vehicle.vehicleType === VehicleType.EV
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${vehicle.status === VehicleStatus.Active ? 'bg-green-100 text-green-800' :
