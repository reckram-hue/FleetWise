...
export function assertCanStartChargingSession(
  ...
  if (vehicle.vehicleType !== 'EV') {
    throw new functions.https.HttpsError('failed-precondition', 'Mid-shift charging is only available for EV vehicles.');
  }
  if (vehicle.activeChargingSessionId) {
    throw new functions.https.HttpsError('failed-precondition', 'This vehicle already has an open mid-shift charging session.');
  }
  if (!location) {
...
export function assertCanEndChargingSession(driverId: string, session: ChargingSessionLike): void {
  if (session.driverId !== driverId) {
    throw new functions.https.HttpsError('permission-denied', 'You can only end your own charging session.');
  }
  if (session.status !== 'OPEN') {
    throw new functions.https.HttpsError('failed-precondition', 'This charging session is not open.');
  }
}
