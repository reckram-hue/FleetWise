...
function harness(overrides = {}) {
  ...
  const realFiles = ['App','vehicleIdentity', 'EvidencePhoto', 'defectVisibility', 'elapsedTime', 'fuelEconomy', 'adminSession', 'AdminLogin', 'ManageDefects', 'TelegramDrivers', 'AccidentReportEntry', 'AccidentReportDetails'];
  function load(filename) {
    ...
    mod.require = name => {
      ...
      if (name.endsWith('/store/shift')) return { shiftStore: { clearActiveShift() {} }, useShiftStore: () => ({ activeShift: null, setActiveShift() {}, clearActiveShift() {} }) };
