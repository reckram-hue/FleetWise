...
describe('FleetWise canonical Firestore rules', () => {
  ...
  test('stale snapshots cannot replace server-owned vehicle fields; filtered edits preserve pointers', async () => {
    ...
    const stale={registration:'EDITED',activeAssignmentId:'old',activeShiftId:'old',activeChargingSessionId:'old',openChargingEventId:'old',status:'Active',currentOdometer:1,lastServiceOdometer:1,lifecycleRevision:0,maintenanceHold:null,lastServiceDate:'2020-01-01'};
    const current={activeAssignmentId:'current-a',activeShiftId:'current-s',activeChargingSessionId:'current-c',openChargingEventId:'current-e',status:'In Service',currentOdometer:1500,lastServiceOdometer:1400,lifecycleRevision:2,maintenanceHold:{id:'current-hold',source:'MANUAL',reason:'Brakes'},lastServiceDate:'2025-01-01'};
    ...
    for(const [key,value] of Object.entries(current)) require('node:assert/strict').deepEqual(saved[key],value);
    await assertFails(activeAdmin().collection('vehicles').doc('vehicle-1').update({'maintenanceHold.id':'injected'}));
    await assertFails(activeAdmin().collection('vehicles').doc('new-hold-injected').set({status:'Active',lifecycleRevision:1,maintenanceHold:{id:'fake'}}));
