...
function Preview() {
  ...
  return <UserContext.Provider value={{ currentUser: { ...driver, role: screen === 'fault' ? 'driver' : 'admin' } as any, setCurrentUser() {} }}>
    ...
    {narrow ? <iframe title="375px mobile preview" src={`/?embedded=1&screen=${screen}${params.has('maintenance') ? '&maintenance=1' : ''}`} style={{ width: 375, height: 812, display: 'block', margin: 'auto', border: 0 }} /> : <div>
