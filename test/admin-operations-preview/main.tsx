...
function Preview() {
  ...
  return <UserContext.Provider value={{ currentUser: { ...driver, role: screen === 'fault' ? 'driver' : 'admin' } as any, setCurrentUser() {} }}>
    {!embedded && <nav className="bg-amber-100 p-3 flex flex-wrap gap-4"><strong>LOCAL SYNTHETIC FIXTURE</strong>
      {['admin', 'history', 'fault'].map(s => <button key={s} className="underline min-h-11" onClick={() => setScreen(s)}>{s}</button>)}
      <button className="underline min-h-11" onClick={() => setNarrow(v => !v)}>Toggle 375px width</button>
    </nav>}
    {narrow ? <iframe title="375px mobile preview" src={`/?embedded=1&screen=${screen}`} style={{ width: 375, height: 812, display: 'block', margin: 'auto', border: 0 }} /> : <div>
      {screen === 'admin' ? <AdminDashboard /> : screen === 'history' ? <InspectionHistory onBack={() => setScreen('admin')} onOpenDefect={() => setScreen('admin')} />
